...
function SP = getSigmaPointApp(varargin)
   ...
   % just for SCTLstat
   Cz_tech_Sigma = estruct.sigma(estruct.phi(beta,SP.B_SP(:,1)));
   dCz_tech_Sigma_dxi = estruct.dsigmadxi(estruct.phi(beta,SP.B_SP(:,1)));
...
%    end
   ...
   if(op_SP.req(5))
      ...
      if compute_derivative == 1
         ...
         SP.dCzdxi(:,:,1:size(dCz_tech_Sigma_dxi,3)) = SP.dCzdxi(:,:,1:size(dCz_tech_Sigma_dxi,3)) + dCz_tech_Sigma_dxi;
      end
   end
